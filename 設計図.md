...
| v2.0.0 | 2025-12-31 | 背景設定、カレンダー表示、予算残額表示追加 |
| v2.1.0 | 2025-12-31 | データバックアップ/復元機能 |
| v2.2.0 | 2025-12-31 | Google AdMob広告対応 |
| v2.2.1 | 2026-01-01 | Web/デスクトップでAdMob初期化を回避（クラッシュ防止） |
...
## 現在の進捗とデバッグ状況（2026-01-01）

### 完了
- Web版（Chrome）でアプリ起動を確認（クラッシュせず表示）
- AdMobはWeb/デスクトップでは初期化しない設計に変更済み（`kIsWeb` + `defaultTargetPlatform`でガード）
- Android向け `app-debug.apk` のビルド成功（`build/app/outputs/flutter-apk/app-debug.apk`）

### 既知の課題（Androidエミュレーター）
- Medium Phone API 36.1 でエミュレーター画面がブラックスクリーンになることがある
- `adb` が `device offline` になりやすく、インストール/起動が不安定
- AVDが二重起動扱いになり「Running multiple emulators with the same AVD」エラーが出ることがある（残プロセスやスナップショットが原因になりやすい）

### 切り分けメモ
- アプリのビルド自体は成功しており、現象はエミュレーター/ADB接続の不安定さに引きずられている可能性が高い
...
- Androidの動作確認（優先順）
  - 実機接続での動作確認（USBデバッグ）
  - 可能なら API 34/35 の新規AVDで再検証（API 36に依存しない切り分け）
- エミュレーター安定化
  - 既存エミュレータープロセスの完全終了→コールドブート（スナップショット無効）
  - `adb devices` で `device` になるまで待機してから `flutter run` / `adb install` を実行
- `flutter analyze` のエラー解消（Exit Code 1 になっているため、内容を確認して修正）
- Androidで広告表示の最終確認（テスト広告が表示されること、例外が出ないこと）
...
│   ├── backup_service.dart      # 【v2.1追加】バックアップ/復元
│   └── ad_service.dart          # 【v2.2追加】広告管理
├── providers/                   # 状態管理
...
│   ├── week_calendar.dart       # 【v2】CalendarScreenに統合済み
│   └── month_calendar.dart      # 【v2】CalendarScreenに統合済み
└── utils/                       # ユーティリティ
    ├── constants.dart           # 定数
    ├── theme.dart               # テーマ設定
...
| share_plus | 【v2.1】ファイル共有 |
| file_picker | 【v2.1】ファイル選択 |
| google_mobile_ads | 【v2.2】広告表示 |
...
### Phase 4: 「カスタマイズする」【v2.0.0】 ✅ 完了
- [x] 背景設定機能（色/画像/グラデーション）
- [x] 月カレンダー表示
- [x] 週カレンダー表示
- [x] 予算残額の大きな表示
- [x] 設定画面の実装
...
### Phase 5: 「データを守る」【v2.1.0】 ✅ 完了
- [x] データバックアップ機能（JSONエクスポート）
- [x] データ復元機能（JSONインポート）
- [x] バックアップファイルの共有機能
- **目標**: データを安全に保存・復元できること

### Phase 6: 「収益化」【v2.2.0】 ✅ 完了
- [x] Google AdMob SDK統合
- [x] バナー広告の下部配置
- [x] テスト広告ID設定
- **目標**: アプリの収益化を可能にすること
- **注意**: 本番リリース前に実際のAdMob広告IDに変更が必要
...
### 広告の本番リリース手順
- [ ] **AdMobアカウント作成**: https://admob.google.com/
- [ ] **アプリ登録**: AdMobコンソールでアプリを登録
- [ ] **広告ユニット作成**: バナー広告ユニットを作成
- [ ] **AndroidManifest.xml更新**: テストApp ID → 本番App ID
  - ファイル: `android/app/src/main/AndroidManifest.xml`
  - 変更: `ca-app-pub-3940256099942544~3347511713` → 実際のApp ID
- [ ] **ad_service.dart更新**: テスト広告ID → 本番広告ユニットID
  - ファイル: `lib/services/ad_service.dart`
  - Android: `ca-app-pub-3940256099942544/6300978111` → 実際の広告ユニットID
  - iOS: `ca-app-pub-3940256099942544/2934735716` → 実際の広告ユニットID
- [ ] **iOS設定追加**: `ios/Runner/Info.plist`にGADApplicationIdentifier追加
- [ ] **テスト実施**: 実機で広告表示を確認
...
- [ ] アプリ説明文の作成
- [ ] プライバシーポリシーの作成
- [ ] リリースビルドの作成 (`flutter build appbundle`)
